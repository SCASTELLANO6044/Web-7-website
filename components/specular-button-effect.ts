...
export type SpecularEffectConfig = {
...
export function createSpecularEffect(button: HTMLButtonElement, fx: HTMLSpanElement, propsRef: { current: SpecularEffectConfig }) {
    ...
    const update = (now: number) => {
      ...
      const steer = current.followMouse && pointerAngle !== null && (!current.autoAnimate || proximityT > 0);
      ...
      const targetBrightness = current.autoAnimate ? 1 : proximityT;
